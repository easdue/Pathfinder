...
public class MyPopupMenu {
    ...
    public void dismissImmediate() {
        MenuPopup popup = mPopup.getPopup();

        if (popup instanceof StandardMenuPopup) {
            ListView listView = ((StandardMenuPopup) popup).getListView();
            View popupView = (View) listView.getParent().getParent();
            //I'm looking from PopupWindow.PopupDecorView but that's private
            WindowManager windowManager = (WindowManager)popupView.getContext().getSystemService(Context.WINDOW_SERVICE);
            windowManager.removeViewImmediate(popupView);
        } else if (popup instanceof CascadingMenuPopup) {
            throw new RuntimeException("Handling of CascadingMenuPopup needs to be implemented");
        }
    }
